...
// sets the search query string
    ...
    public function query($offset) {
        $this->current = null;
        $this->offset = $offset;
        // to do: retrieve the correct page. right now only first page
        if (!$this->get_results()) {
...
// sets the maximum results
     ...
     * @return element | bool when FALSE
     */
...
// sets the api version like '/v0'. no trailing slash
    ...
    private function get_results() {
        ...
        $data['from'] = $this->offset  + ($this->page_num() * $this->size);
        $data['size'] = isset($this->limit) ? (
                $this->offset + $this->limit < $data['from'] + $this->size ?
